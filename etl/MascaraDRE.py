...
def extract_data(filepath):
    """
    Lê a Tabela MascaraDRE do Excel.
    """
    print(f"Lendo arquivo: {filepath}")
    df = pd.read_excel(filepath, sheet_name="MascaraDRE")
    return df
...
def load_data(conn, df):
    """
    Realiza o TRUNCATE e o INSERT na tabela dMascaraDRE.
    """
    cursor = conn.cursor()
    ...
    try:
        ...
        cursor.execute("TRUNCATE TABLE public.dMascaraDRE;")

        # 2. Definição da Ordem das Colunas
        colunas_ordem = ['CodGrupoDRE', 'ContaGerencial', 'Subtotal']
        ...
        # 4. SQL de INSERT 
        sql_insert = """
        INSERT INTO public.dMascaraDRE (
            CodGrupoDRE,
            ContaGerencial,
            Subtotal
        ) VALUES (%s, %s, %s)
        ON CONFLICT (CodGrupoDRE, ContaGerencial) DO NOTHING;
        """
