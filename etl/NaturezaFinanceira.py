...
def extract_data(filepath):
    """
    Lê a Tabela naturezaFinanceira do Excel.
    """
    print(f"Lendo arquivo: {filepath}")
    df = pd.read_excel(filepath, sheet_name="NaturezaFinanceira")
    return df
...
def load_data(conn, df):
    ...
    try:
        # 1. Limpeza
        print("Limpando tabela dNaturezaFinanceira...") # Corrigido o texto do log
        cursor.execute("TRUNCATE TABLE public.dNaturezaFinanceira;")

        # 2. Definição da Ordem das Colunas
        colunas_ordem = ['CodNatFinanceira', 'NaturezaFinanceira', 'TipoNatureza']
        ...
        # 4. SQL de INSERT 
        sql_insert = """
        INSERT INTO public.dNaturezaFinanceira (
            CodNatFinanceira,
            NaturezaFinanceira,
            TipoNatureza
        ) VALUES (%s, %s, %s)
        ON CONFLICT (CodNatFinanceira) DO NOTHING;
        """
